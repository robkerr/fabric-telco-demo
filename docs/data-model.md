...
|---|---|---|
| `OutageEvents` | one outage impact per customer | `event_id`, `customer_id`, `geo_id`, `event_time`, `outage_type`, `severity`, `status`, `affected_service`, `duration_minutes`, `restored_time`, `reported_by_customer` |
...
These are bound into the Fabric IQ ontology and related to `Customer`/`Geography` so the
ontology Data Agent can traverse them — see [`../fabric/eventhouse/README.md`](../fabric/eventhouse/README.md).
