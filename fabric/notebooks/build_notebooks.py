...
def nb_05():
    ...
    return notebook([
        ...
        code(f"NAME = {cfg['name']!r}",
             ...
             f"GOLD_SCHEMA = {GOLD!r}",
             f"GOLD_TABLES = {GOLD_TABLES!r}",
        ...
        md("## Create, configure, and publish",
           "",
           "The datasource is added to **staging**. `add_staging_datasource()` returns a raw",
           "`Response`, and `get_datasources()` returns only *published* datasources (empty until",
           "you publish), so we fetch the configurable **staging** `Datasource` object via the",
           "staging-aware getter. If the SDK version doesn't expose one, the notebook still",
           "publishes and you select the gold tables in the Data Agent UI."),
        ...
        code("def _configurable_datasource(agent):",
             "    \"\"\"Return a staging Datasource object that supports .select(), or None.\"\"\"",
             "    cands = []",
             "    for _g in ('get_staging_datasources', 'list_datasources', 'get_datasources'):",
             "        _fn = getattr(agent, _g, None)",
             "        if not callable(_fn):",
             "            continue",
             "        try:",
             "            _res = _fn()",
             "        except Exception:",
             "            continue",
             "        _items = list(_res) if isinstance(_res, (list, tuple)) else [_res]",
             "        cands += [it for it in _items if hasattr(it, 'select')]",
             "    _client = getattr(agent, '_client', None)",
             "    if not cands and _client is not None:",
             "        for _g in ('get_staging_datasources', 'get_datasources'):",
             "            _fn = getattr(_client, _g, None)",
             "            if not callable(_fn):",
             "                continue",
             "            try:",
             "                _res = _fn()",
             ...
             "",
             "# add_datasource() returns a configurable Datasource directly; add_staging_datasource()",
             "# returns a Response, so we then fetch the staging Datasource via a getter.",
             "ds = None",
             "if hasattr(agent, 'add_datasource'):",
             "    try:",
             "        _res = agent.add_datasource(",
             "            artifact_name_or_id=LAKEHOUSE, workspace_id_or_name=workspace_id)",
             "        if hasattr(_res, 'select'):",
             "            ds = _res",
             "    except Exception as _ex:",
             "        print('add_datasource unavailable, trying staging flow:', _ex)",
             "if ds is None:",
             "    try:",
             "        agent.add_staging_datasource(",
             "            artifact_name_or_id=LAKEHOUSE, workspace_id_or_name=workspace_id)",
             "    except Exception as _ex:",
             "        print('add_staging_datasource note:', _ex)",
             "    ds = _configurable_datasource(agent)",
             "",
             "if ds is None:",
             "    print('NOTE: could not obtain a configurable Datasource object from this SDK version.')",
             "    print('      The datasource is added; SELECT the gold tables in the Data Agent UI')",
             "    print('      (Explorer -> check the gold schema), then Publish. Continuing to publish.')",
             "else:",
             "    print('Got configurable datasource for', LAKEHOUSE, '->', type(ds).__name__)"),
        code("# Show the element tree so you can see valid table paths.",
             "if ds is not None:",
        ...
        code("if ds is not None:",
             ...
             "    except Exception:",
             "        pass"),
        code("# Publish (method name depends on the flow used above).",
             "_published = False",
             "for _p in ('publish_staging', 'publish'):",
             "    _fn = getattr(agent, _p, None)",
             "    if not callable(_fn):",
             "        continue",
             "    try:",
             "        _fn(description=DESCRIPTION)",
             "        print(f'Published data agent via {_p}().')",
             "        _published = True",
             "        break",
             "    except Exception as _ex:",
             "        print(f'{_p}() failed: {_ex}')",
             "if not _published:",
             "    print('Could not publish automatically - click Publish in the Data Agent UI.')"),
