...
def nb_05():
    ...
    return notebook([
        ...
        md("## Create, configure, and publish",
           "",
           "Uses the confirmed SDK methods: `ds.select(schema, table)` to choose which tables the",
           "agent can query (this is what populates the table list in the UI), `ds.update_configuration`",
           "for datasource instructions, and `ds.add_fewshots` for example queries."),
        code("from fabric.dataagent.client import create_data_agent",
             ...
             "",
             "# Select the gold tables so the agent can query them (fixes 'no tables selected').",
             "selected = 0",
             "for _t in GOLD_TABLES:",
             "    try:",
             "        ds.select(GOLD_SCHEMA, _t)",
             "        selected += 1",
             "    except Exception as _ex:",
             "        print(f'  could not select {GOLD_SCHEMA}.{_t}: {_ex}')",
             "if selected == 0:",
             "    # fallback: select the whole gold schema",
             "    try:",
             "        ds.select(GOLD_SCHEMA)",
             "        print('Selected the entire gold schema (per-table select was unavailable).')",
             "    except Exception as _ex:",
             "        print('Could not select gold tables automatically:', _ex)",
             "        print('Use ds.pretty_print() output below to pick the correct path.')",
             "else:",
             "    print(f'Selected {selected}/{len(GOLD_TABLES)} gold tables.')",
             "",
             "# Datasource-level instructions.",
             "if DS_INSTRUCTIONS:",
